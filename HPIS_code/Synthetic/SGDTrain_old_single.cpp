...
int main(int argc, char* argv[]) {
  ...
  int *fkidx_arr;
  int num_r, ep, num_p; // epochs, partitions
  // only used for HP (mode 3 or 4), 0 when not used
  num_r = num_p = 0; // num of partitions
  ...
  int *ridx, *old_ridx; // row index array only for HP.
  int seg = -1;
  ridx = old_ridx = NULL;

  if (mode == 3 || mode == 4) {
    num_p = min(num_p, ns);
    ridx = gen_ridx(num_p, ns, seg); // free
    assert(seg != -1);
    old_ridx = new int[num_p];
    memcpy(old_ridx, ridx, sizeof(int)*num_p);
  }
...
ofstream lossTblFile(string(output_root) + string("/lossvalue.tbl"));
  switch (mode) { 
	...
	if (debug) {
	  ...
	  cout << "Loss value @ epoch " << i+1 << ": " << loss_value << endl;
	}
	if (i == ep - 1) {
	  loss_value = lossfunc(T, dw, W);
	}
    ...
    case 3: // HPIS shuffleAlways
      if (debug) cout << "ShuffleAlways HPIS:\n";
      assert(num_p != 0 && seg != -1 && ridx && old_ridx); 
      for (int i = 0; i < ep; ++i) {
	// shuffle within chunks always	
	memcpy(ridx, old_ridx, sizeof(int)*num_p);
	hp_shuffle(S, ridx, num_p, ns);
	...
	for (int j = 0; j < num_p; ++j) {
	  int rnum = min(seg, ns-ridx[j]);

	  if (debug) {
	    cout << "training based on row " << ridx[j]
		 << " to row " << ridx[j]+rnum-1 << endl;
	  }
...
int *gen_ridx(int p_num, int total, int &seg) {
  seg = ceil(total * 1.0 / p_num);
  double exact_seg = total * 1.0 / p_num;
  assert(total > p_num*(seg-1) && total <= p_num*seg);
    
  if (debug) {
    cout << "Chunk size: " << seg << endl;
  }
  int *result = new int[p_num];

  if (debug) cout << "row idx array: ";
    int offset = 0;
  for (int i = 0; i < p_num; ++i) {
    result[i] = i*seg - offset;
    // Re-scale the index
    if (result[i] - exact_seg * i > 1) {
	result[i] -= 1;
	offset += 1;
    }
    if (debug) {
